...
module.exports = class Likes extends Model {
    ...
    async getLikesCount(obj) {
        ...
        let sql = `SELECT count(*) as dislikes FROM likes LEFT JOIN ${pre}s_likes ON likes.id = ${pre}s_likes.like_id WHERE ${pre}s_likes.${pre}_id = ${obj.post_id ?? obj.comment_id} and likes.type = FALSE;`
        let res = (await connection.promise().query(sql))[0];
        ...
        sql = `SELECT count(*) as likes FROM likes LEFT JOIN ${pre}s_likes ON likes.id = ${pre}s_likes.like_id WHERE ${pre}s_likes.${pre}_id = ${obj.post_id ?? obj.comment_id} and likes.type = TRUE;`
        res = (await connection.promise().query(sql))[0];
        if (!res.length) {
            return -1;
        }
        counts = {...counts, ...res[0]};
        counts.total = counts.likes + counts.dislikes;
        connection.end();
